...
// 2D line/line intersection in screen space (returns t along first segment)
...
// Build triangle adjacency, but DO NOT connect triangles across edges in cutEdges.
void buildTriangleAdjacency(std::vector<std::vector<int>>& adjList,
    ...
    for (int i = 0; i < numTris; i++) {
        ...
        for (int e = 0; e < 3; e++) {
            ...
            // Don't glue triangles together across cut edges.
            if (cutEdges.count(edge)) continue;
...
void performCut(int sx0, int sy0, int sx1, int sy1) {
    ...
    // 1) Find all intersections between the mouse segment and triangle edges.
    ...
    // For each UNIQUE original edge, create a new vertex at the intersection point.
    std::set<std::pair<int, int>> cutEdgesOriginal;
    ...
    for (const auto& ip : intersections) {
        ...
        std::pair<int, int> edge(va, vb);

        if (edgeToNewVert.count(edge)) continue; // already split this edge
        ...
        cutEdgesOriginal.insert(edge);
    }

    printf("New vertices on cut edges: %d\n", (int)edgeToNewVert.size());
    printf("Original cut edges: %d\n", (int)cutEdgesOriginal.size());

    // 2) Re-triangulate all triangles that were touched by the cut.
    ...
    for (int triIdx : affectedTris) {
        if (triIdx < 0 || triIdx >= (int)triangles.size()) continue;

        Triangle& tri = triangles[triIdx];
        if (!tri.active) continue;
        tri.active = false; // we'll replace it
        ...
        int splitCount = (vab >= 0 ? 1 : 0) +
                         (vbc >= 0 ? 1 : 0) +
                         (vca >= 0 ? 1 : 0);

        if (splitCount == 0) {
            // shouldn't happen for affectedTris, but keep safe
            tri.active = true;
            continue;
        }
        else if (splitCount == 1) {
            // Triangle cut only on one edge -> split into 2
            if (vab >= 0) {
            ...
            // Triangle cut by two edges -> split into 3
            if (vab >= 0 && vbc >= 0) {
            ...
            // Cut passes through all three edges -> 4 new triangles around the centroid
            newTriangles.push_back({ va, vab, vca, true });
    ...
    printf("New triangles created: %d\n", (int)newTriangles.size());

    // 3) Build adjacency *after* re-triangulation, explicitly blocking
    //    the segments along the cut.  For each original cut edge (a,b)
    //    with intersection vertex nv, we block edges (a,nv) and (nv,b).
    std::set<std::pair<int, int>> blockedEdges;
    for (const auto& kv : edgeToNewVert) {
        int a = kv.first.first;
        int b = kv.first.second;
        int nv = kv.second;

        blockedEdges.insert(std::make_pair(std::min(a, nv), std::max(a, nv)));
        blockedEdges.insert(std::make_pair(std::min(b, nv), std::max(b, nv)));
    }

    std::vector<std::vector<int>> adjList;
    buildTriangleAdjacency(adjList, blockedEdges);

    // 4) Connected components of the triangle graph give us separate pieces.
    ...
    // 5) Duplicate boundary vertices that belong to multiple components,
    //    and offset them slightly so the cut visually opens.
    if (numComponents > 1) {
        Vec3 cutDir = computeCutDirection(sx0, sy0, sx1, sy1);

        // For each vertex, which components does it appear in?
        ...
        // For each (vertex, component) pair, which vertex index should we use?
        std::map<std::pair<int, int>, int> vertCompToNew;

        for (auto it = vertToComps.begin(); it != vertToComps.end(); ++it) {
            int vertIdx = it->first;
            const std::set<int>& comps = it->second;

            if (comps.size() <= 1) continue; // only boundary vertices are duplicated

            std::vector<int> compVec(comps.begin(), comps.end());

            // Original vertex stays with the first component
            vertCompToNew[std::make_pair(vertIdx, compVec[0])] = vertIdx;

            // All other components get a duplicated vertex, offset along the cut normal.
        ...
        int duplicatedCount = (int)vertCompToNew.size() - (int)vertToComps.size();
        if (duplicatedCount < 0) duplicatedCount = 0;
        printf("Duplicated boundary vertices: %d\n", duplicatedCount);

        // Re-assign triangle vertices according to their component.
        for (size_t i = 0; i < triangles.size(); ++i) {
            if (!triangles[i].active || component[i] < 0) continue;
    ...
    // 6) Rebuild edges from the (possibly updated) triangle list.
    edges.clear();
    ...
    for (const auto& e : edgeSet) {
        edges.push_back({ e.first, e.second, true });
    }

    // 7) Rebuild springs along all edges (the seam is naturally handled
    //    because duplicated vertices break the connectivity).
    springs.clear();
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Cloth Cutting (cut + retriangulate)");
